(ns fontawesome.icons
  (:require [clojure.java.io :as io]
            [clojure.string :as str]))

(def base-path "fontawesome-icons")

(defn get-icon-path [id]
  (str base-path "/"
       (str/replace (namespace id) #"^fontawesome\." "") "/"
       (name id) ".edn"))
...
(defn get-icon-ids* []
  (->> (io/resource base-path)
       ...
       (map #(let [[style id] (str/split % #"/")]
               (keyword (str "fontawesome." style) (str/replace id #"\.edn$" ""))))
       sort
       vec))
...
(defmacro load-all-icons []
  (apply list
         'do
         (for [id (get-icon-ids*)]
           (list 'fontawesome.icons/icon id))))
